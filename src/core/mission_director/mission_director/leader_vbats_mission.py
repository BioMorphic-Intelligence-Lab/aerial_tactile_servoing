...
class MissionDirector(UAMStateMachine):
    def __init__(self):
        ...
        self.sub_leader = self.create_subscription(PoseStamped, '/mocap/leader/pose', self.leader_callback, 10)
    ...
    def execute(self):
        match self.FSM_state:
            ...
            case "pre_contact_uam_position":
                self.state_move_uam_to_position(target_position=[0.0, 1.0, -1.3], next_state="pre_contact_arm_position")
            ...
            case "tactile_servoing":
                ...
                self.get_logger().info(f'Contact depth: {self.tactip_data.twist.linear.z:.2f} mm', throttle_duration_sec=1)

    ...
    def execute(self):
        match self.FSM_state:
            ...
            case "approach": # Better way is to command a negative z velocity on the end-effector and run it through the inverse kinematics
                self.state_approach_wall_position(approach_speed=[0.0, 0.05, 0.0], approach_heading=np.deg2rad(-15.), transition=self.contact, next_state="tactile_servoing")
            ...
            case "tactile_servoing":
                ...
                rotation_ee = self.get_transform().transform.rotation
                ...
                rotation_ee_euler = R.from_quat([rotation_ee.x, rotation_ee.y, rotation_ee.z, rotation_ee.w]).as_euler('xyz', degrees=True)
                self.get_logger().info(f'End-effector orientation (Euler angles): roll={rotation_ee_euler[0]:.1f}, pitch={rotation_ee_euler[1]:.1f}, yaw={rotation_ee_euler[2]:.1f}', throttle_duration_sec=1)
    ...
    def get_transform(self):
        try:
            transform = self.tf_buffer.lookup_transform(
                'world',   # in this frame
                'present_sensor_frame',   # display this frame
                rclpy.time.Time()
            )
